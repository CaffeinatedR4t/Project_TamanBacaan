        ...
        bottomNav.setOnItemSelectedListener { item ->
            when (item.itemId) {
                R.id.nav_admin_books -> {
                    loadFragment(BookManagementFragment()) // Req: CRUD Buku
                    true
                }
                R.id.nav_admin_transactions -> {
                    loadFragment(TransactionManagementFragment()) // Req: Kelola Transaksi, Lihat Anggota
                    true
                }
                R.id.nav_admin_reports -> {
                    loadFragment(ReportFragment()) // Req: Statistik, Laporan Bulanan
                    true
                }
                else -> false
            }
